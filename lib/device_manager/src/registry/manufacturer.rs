...
/// This function will return the `manufacturer` value stored in the registry to you as a result (possible Ok(String) or Err(RegistryOutcomes)).
/// 
/// ## Example Usage
/// ```rust
...
/// fn main() {
///     // Get Asset Tag.
///     let possible_manufacturer = device_manager::registry::manufacturer::get();
/// 
///     if possible_manufacturer.is_err() { println!("{}", possible_manufacturer.unwrap())}
///     else { println!("{}", possible_manufacturer.unwrap()); }
///
/// }
/// ```
pub fn get() -> Result<String, RegistryOutcomes> {
    let key_name = "manufacturer".to_string();
    ...
    let manufacturer = current_version.unwrap().get_value(key_name).unwrap_or("Null".to_string());
    Ok(manufacturer)
}
...
fn test_get() {
     // Get Asset Tag.
     let possible_manufacturer = get();
    assert!(possible_manufacturer.is_ok());
}
...
/// 
/// fn main() {
///     let old_manufacturer = device_manager::registry::manufacturer::get().unwrap();///  
///     println!("{}", old_manufacturer);
/// 
///     println!("Updating Asset Tag Value to 12345");
///     let manufacturer_updated = device_manager::registry::manufacturer::set("12345".to_string());
///     
///     if manufacturer_updated.is_ok() { println!("The Asset Tag Value in the registry has been successfully updated!"); }
...
/// 
///     println!("Asset Tag Value has been updated to {}", device_manager::registry::manufacturer::get().unwrap());
/// }
/// ```
