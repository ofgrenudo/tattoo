...
/// This function will return the `model` value stored in the registry to you as a result (possible Ok(String) or Err(RegistryOutcomes)).
/// 
/// ## Example Usage
/// ```rust
...
/// fn main() {
///     // Get Asset Tag.
///     let possible_model = device_manager::registry::model::get();
/// 
///     if possible_model.is_err() { println!("{}", possible_model.unwrap())}
///     else { println!("{}", possible_model.unwrap()); }
///
/// }
/// ```
pub fn get() -> Result<String, RegistryOutcomes> {
    let key_name = "model".to_string();
    ...
    let model = current_version.unwrap().get_value(key_name).unwrap_or("Null".to_string());
    Ok(model)
}
...
fn test_get() {
     // Get Asset Tag.
     let possible_model = get();
    assert!(possible_model.is_ok());
}
...
/// 
/// fn main() {
///     let old_model = device_manager::registry::model::get().unwrap();///  
///     println!("{}", old_model);
/// 
///     println!("Updating Asset Tag Value to 12345");
///     let model_updated = device_manager::registry::model::set("12345".to_string());
///     
///     if model_updated.is_ok() { println!("The Asset Tag Value in the registry has been successfully updated!"); }
...
/// 
///     println!("Asset Tag Value has been updated to {}", device_manager::registry::model::get().unwrap());
/// }
/// ```
