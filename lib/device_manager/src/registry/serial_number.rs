...
/// This function will return the `serial_number` value stored in the registry to you as a result (possible Ok(String) or Err(RegistryOutcomes)).
/// 
/// ## Example Usage
/// ```rust
...
/// fn main() {
///     // Get Asset Tag.
///     let possible_serial_number = device_manager::registry::serial_number::get();
/// 
///     if possible_serial_number.is_err() { println!("{}", possible_serial_number.unwrap())}
///     else { println!("{}", possible_serial_number.unwrap()); }
///
/// }
/// ```
pub fn get() -> Result<String, RegistryOutcomes> {
    let key_name = "serial_number".to_string();
    ...
    let serial_number = current_version.unwrap().get_value(key_name).unwrap_or("Null".to_string());
    Ok(serial_number)
}
...
fn test_get() {
     // Get Asset Tag.
     let possible_serial_number = get();
    assert!(possible_serial_number.is_ok());
}
...
/// 
/// fn main() {
///     let old_serial_number = device_manager::registry::serial_number::get().unwrap();///  
///     println!("{}", old_serial_number);
/// 
///     println!("Updating Asset Tag Value to 12345");
///     let serial_number_updated = device_manager::registry::serial_number::set("12345".to_string());
///     
///     if serial_number_updated.is_ok() { println!("The Asset Tag Value in the registry has been successfully updated!"); }
...
/// 
///     println!("Asset Tag Value has been updated to {}", device_manager::registry::serial_number::get().unwrap());
/// }
/// ```
